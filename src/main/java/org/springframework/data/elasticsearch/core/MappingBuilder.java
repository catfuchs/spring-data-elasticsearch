...
class MappingBuilder {
    ...
    private static void addSingleFieldMapping(XContentBuilder xContentBuilder, java.lang.reflect.Field field,
        ...
        if (FieldType.Auto != fieldAnnotation.type()) {
            ...
            } else if (FieldType.String == fieldAnnotation.type()) {
                if (-1 != fieldAnnotation.ignoreAbove()) {
                    xContentBuilder.field(FIELD_IGNORE_ABOVE, fieldAnnotation.ignoreAbove());
                }
            }
        }
        ...
        if (FieldIndexOptions.Auto != fieldAnnotation.indexOptions()) {
            xContentBuilder.field(FIELD_INDEX_OPTIONS, fieldAnnotation.indexOptions().name().toLowerCase());
        }
        if (FieldSimilarity.Auto != fieldAnnotation.similarity()) {
            xContentBuilder.field(FIELD_SIMILARITY, fieldAnnotation.indexOptions().name().toLowerCase());
        }
        if (FieldTermVector.Auto != fieldAnnotation.termVector()) {
            xContentBuilder.field(FIELD_TERM_VECTOR, fieldAnnotation.termVector().name().toLowerCase());
        }
        ...
        if (null != fieldAnnotation.copyTo() && fieldAnnotation.copyTo().length > 0) {
            if (1 == fieldAnnotation.copyTo().length) {
                xContentBuilder.field(FIELD_COPY_TO, fieldAnnotation.copyTo()[0]);
            } else {
                xContentBuilder.field(FIELD_COPY_TO, fieldAnnotation.copyTo());
            }
        }
        if (!Double.isNaN(fieldAnnotation.boost())) {
            xContentBuilder.field(FIELD_BOOST, fieldAnnotation.boost());
        }
        if (!fieldAnnotation.coerce()) {
            xContentBuilder.field(FIELD_COERCE, fieldAnnotation.coerce());
        }
        if (!fieldAnnotation.ignoreMalformed()) {
            xContentBuilder.field(FIELD_IGNORE_MALFORMED, fieldAnnotation.ignoreMalformed());
        }
        if (!fieldAnnotation.includeInAll()) {
            xContentBuilder.field(FIELD_INCLUDE_IN_ALL, fieldAnnotation.includeInAll());
        }
        if (isNotBlank(fieldAnnotation.nullValue())) {
            xContentBuilder.field(FIELD_NULL_VALUE, fieldAnnotation.nullValue());
        }
        if (-1 == fieldAnnotation.positionIncrementGap()) {
            xContentBuilder.field(FIELD_POSITION_INCREMENT_GAP, fieldAnnotation.positionIncrementGap());
        }
